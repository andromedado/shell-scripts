...
function isRunning(processRegExp) {
    try {
        childProcess.execSync(`ps ax | grep '${processRegExp}'`);
        return true;
    } catch (e) {
        return false;
    }
...
function ensureOff(what, processRegExp, shutdownCommand) {
    if (isRunning(processRegExp)) {
        info(`Stopping ${what}`);
        if (shutdownCommand) {
            childProcess.execSync(shutdownCommand);
        }
        childProcess.execSync(`ps -eaf | grep '${processRegExp}' | awk '{print "kill -9 " $2}' | sh`);
    } else {
...
function ensureRunning(what, processRegExp, startupCommand) {
    if (!isRunning(processRegExp)) {
        info(`Starting ${what}: \`${startupCommand}\``);
        childProcess.execSync(startupCommand);
    } else {
